...
{
    class DataServer2 : ServiceModuleBase
    {
            ...
            ) : base(logger)
        {
            ...
            this.TripOutputProvider = tripOutputProvider;
            ...
            TripOutputProvider.IsOutputBeforeTemperatureCompensation = false;
            tripOutputProvider.RawPartitionSize = DataFileOption.UserPartitionSize;
            //TripOutputProvider.IsAllowFiltering = false;
        ...
        {
            this.RemoteDataPointResultRec.Write("staticHanlder", "DataServer", dataPointResult);
        ...
        {
            this.RemoteDataPointResultRec?.Write("cmp_shallowZone", "shallowZoneHandler", dataPointResult);
        ...
        {
            this.RemoteDataPointResultRec?.Write("cmp_shallowZoneX", "shallowZoneXHandler", dataPointResult);
            if (!IsDataAnalysisAlwaysUseY)
            {
                List<ShallowZoneSegment> shallowZoneSegments = ShallowZoneExtractAlgorithm.ExtractShallowZoneSegments(dataPointResult, ShallowZoneExtractAlgorithm.CreateOptions(DataChannel.Ray, TripOutputProvider.Parameters));
        ...
        {
            RemoteDataPointResultRec?.Write("rawArrived", "DataServer", dataPointResult);
        ...
        {
            this.RemoteDataPointResultRec?.Write("cmpMainTrip", "beforeDealedSurfaceDensityArrived", dataPointResult);
            var values = new float[DataFileOption.PartitionCount];
            var dataPoints = dataPointResult.DataPoints;
        ...
        private ResponsePackage packageHandler(RequestPackage requestPackage, string remoteIP)
        {
            ...
            switch (requestPackage.Header)
            {
                ...
                case "MEASUREMENTCONTROL"://禁止开机(启动扫描)命令
                    if (IsRemoteControl)
                    {
                        var g = System.BitConverter.ToInt32(requestPackage.Data, 0);
        ...
        protected override Task<ServiceCoreResult> StartCore()
        {
            return Task.Run(() =>
            {
                ...
                _shallowZoneXHandler =  RequireFilterResults.RegisterOrderedHandler(shallowZoneXHandler);
                ...
                if (TripOutputProvider is TestTripOutputProvider tpder)
                    tpder.IsTest = true;

                //test
                TripOutputProvider.IsAllowFiltering = true;
                //TripOutputProvider.NotRemoveNonsensePoints = true;
                TripOutputProvider.IsRawAllowPositionFilter = true;
                TripOutputProvider.IsOutputRawTripData = true;
                TripOutputProvider.RawTripHandler+= (dps) => this.RemoteDataPointResultRec?.Write("cmpMainTrip", "RawTripHandler", dps);

                TripOutputProvider.IsOutputShallowZone = true;
                TripOutputProvider.ShallowZoneHandler += (dps) => this.RemoteDataPointResultRec?.Write("cmp_shallowZone", "shallowZoneHandler_Trip", dps);

                TripOutputProvider.IsOutputShallowZone = true;
                //TripOutputProvider.IsDataAnalysisAlwaysUseY = false;
                TripOutputProvider.ShallowZoneXHandler += (dps) => this.RemoteDataPointResultRec?.Write("cmp_shallowZoneX", "shallowZoneXHandler_Trip", dps);
