...
namespace KFCK.ThicknessMeter.MyTest
{
    ...
    /// </summary>
    internal class DataServerTripOut1 : IAutoExecutor
    {
            ...
            )
        {
            ...
            TripOutputProvider.IsOutputRawTripAbsorb = true;
            TripOutputProvider.IsOutputRawTripData = true;
            TripOutputProvider.IsOutputShallowZone = false;
            TripOutputProvider.IsOutputTripAbsorb = false;
        ...
        //    var tmp = FilterConfigs.createTripOutputCommonBaseConfig();
        //    return tmp;
        //}
        /// <summary>
        /// 横向趋势参考趟数
        ...
        public void Start()
        {
            ...
            {
                DebugInfo("");
                this.DataPointResultRec.Start("dataserverTripOut1");
                TripOutputProvider.RawTripHandler += rawArrived;
                TripOutputProvider.AfterAutoReverseItHandler += AutoReverseItHandle;
                TripOutputProvider.ZeroPointsAppliedHandler += ZeroPointsAppliedHandler;
                TripOutputProvider.RawTripAbsorbHandler += RawTripAbsorbHandler;
                //TripOutputProvider.AfterPreProcessingHandler += AfterPreProcessingHandler;
                //TripOutputProvider.TripHandler += beforeDealedSurfaceDensityArrived;
                //TripOutputProvider.ShallowZoneHandler += shallowZoneHandler;
                //TripOutputProvider.ShallowZoneXHandler += shallowZoneXHandler;
                //TripOutputProvider.TripHandler += surfaceDensitiyArrived;
                //TripOutputProvider.TripHandler += dealData;
                TripOutputProvider.Start();
                return default;
            });
        }
        void RawAfterPostionFiterHandler(DataPointResult dataPointResult)
        {
            this.DataPointResultRec.Write("RawAfterPostionFiterHandler", "TripOut1", dataPointResult);
        }
        void RawTripAbsorbHandler(DataPointResult dataPointResult)
        {
            this.DataPointResultRec.Write("RawTripAbsorbHandler", "TripOut1", dataPointResult);
        }
        void ZeroPointsAppliedHandler(DataPointResult dataPointResult)
        {
            this.DataPointResultRec.Write("ZeroPointsAppliedHandler", "TripOut1", dataPointResult);
        }
        //翻转之后数据个数是相同的
        void AutoReverseItHandle(DataPointResult dataPointResult)
        {
            this.DataPointResultRec.Write("AutoReverseItHandle", "TripOut1", dataPointResult);
        }
        void AfterPreProcessingHandler(DataPointResult dataPointResult)
        {
            this.DataPointResultRec.Write("AfterPreProcessingHandler", "TripOut1", dataPointResult);
        }
        ...
        {
            DataPointResultRec.Write("rawArrived","TripOut1", dataPointResult);
