...
namespace HslCommunicationDemo
{
	public partial class FormS7Server : HslFormContent
	{
		...
		private void FormSiemens_Load( object sender, EventArgs e )
		{
            ...
            this.bt_import.Click += Bt_import_Click;
		}

        private void Bt_import_Click(object sender, EventArgs e)
        {
			var of = new OpenFileDialog();
			of.Filter = "DbList|*.json";
			if (of.ShowDialog()== DialogResult.OK)
			{
				var f = of.FileName;
				try
				{
                    var lst = JsonConvert.DeserializeObject<List<string>>(System.IO.File.ReadAllText(f));
					lst.ForEach(a =>
					{
						var b = a.ToLower();
						if (b.StartsWith("db"))
						...
						s7NetServer?.AddDbBlock(Int32.Parse(b));
					});
                }catch(Exception ex)
				{
